...
type Thinking struct {
	...
	// caller that names a level does not also need this (see [Thinking.Active]).
	Enabled bool
	// BudgetTokens is the Anthropic extended-thinking token budget (0 = derive
	// one from Effort, or the vendor floor when no Effort is set either). It is
	// Anthropic's native unit and a more precise statement than a level, so a
	// non-zero value WINS over Effort when both are set — including over a level
	// later installed by Runner.SetEffort. Leave it zero to let effort drive
	// the budget. OpenAI has no budget concept and ignores this field.
	BudgetTokens int
	// Effort is the unified reasoning effort: "low", "medium", or "high" (see
	// [ValidEffort]). OpenAI sends it as its effort string; Anthropic projects
	// it down to a thinking budget. Empty means "no explicit level".
	Effort string
}

// Active reports whether extended reasoning is on for this configuration: it is
// on when the caller either turned it on outright (Enabled) or named a level to
// run it at (a non-empty Effort).
//
// Treating a level as self-enabling is what makes Runner.SetEffort reach the
// wire. The two fields are not independent axes — "run at high effort, but with
