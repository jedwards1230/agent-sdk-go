...
// supportedProviders are the provider ids this package knows how to run
// against, in a fixed, deterministic (alphabetical) order. Both the
// credential-presence probe and the default-model table below are keyed off
// this list, so runner never favors one vendor over another — there is no
// flagship default.
var supportedProviders = []string{"anthropic", "openai"}

// SupportedProviders returns the provider ids runner knows how to run
...
// defaultModels is the model runner uses when a caller doesn't pick one and
...
// EnvVar returns the API-key environment variable this package's credential
...
// currently have some credential configured — either a stored login entry
// or the provider's API-key environment variable — sorted in
