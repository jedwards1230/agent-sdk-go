...
func TestUnmarshalRoundTrip(t *testing.T) {
	...
	cases := []struct {
		...
		{"session.compaction_started", event.NewSessionCompactionStarted(sid, "entry-9", 4)},
		{"session.compacted", event.NewSessionCompacted(sid, "entry-9", 4, "test-model",
			provider.Usage{InputTokens: 500, OutputTokens: 80}, "condensed summary")},
		{"session.compaction_failed", event.NewSessionCompactionFailed(sid, "entry-9", 4,
			"runner: compact session s: summarizer unavailable")},
